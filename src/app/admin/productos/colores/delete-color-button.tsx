...
export function DeleteColorButton({ colorId }: { colorId: string }) {
  ...
  function handleDelete() {
    startTransition(async () => {
      const err = await deleteColor(colorId);
      if (err) setError(err);
    });
  }
  ...
  return (
    <div className="flex flex-col gap-2 border border-ink bg-base p-3">
      <p className="field-label">¿Eliminar este color? No se puede deshacer.</p>
