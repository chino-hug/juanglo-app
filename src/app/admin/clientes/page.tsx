...
export default async function AdminClientesPage({
  ...
  const [clients, facetClients, zones] = await Promise.all([
    listClientsForAdmin({ q: params.q, status, city, region }),
    listClientsForAdmin({ q: params.q, status }),
    listZones(),
  ]);
    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <h1 className="display mt-2 text-3xl">Clientes</h1>
          <p className="mt-1 text-xs text-concrete">
            {clients.length} cliente{clients.length === 1 ? "" : "s"} · Directorio de todo el equipo
            de ventas
          </p>
        </div>
        <Link href="/admin/clientes/zonas" className="font-mono text-xs text-concrete underline">
          Zonas
        </Link>
      </div>
