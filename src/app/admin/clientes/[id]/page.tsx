...
import { convertToClientAdmin } from "../actions";
...
export default async function AdminClienteDetailPage({
  ...
  const seller = users.find((u) => u.id === client.seller_id);

  async function convert() {
    "use server";
    await convertToClientAdmin(id);
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        {seller && <p className="mt-1 text-xs text-concrete">Vendedor/a: {seller.full_name}</p>}
      </div>
      ...
      <section className="flex flex-col gap-2.5">
        ...
        {orders.map((order) => (
          <Link key={order.id} href={`/preparacion/${order.id}`}>
